...
function App() {
  ...
  //Функція для долавання нового контакту
  const addContact = (name, phoneNumber, email) => {
    const newContact = { name, phoneNumber, email };
    setContacts(prevContacts => [...prevContacts, newContact]); 
  };
  
  //Функція для додавання нової зустрічі 
  const addAppointment = (name, contact, dateTime) => {
    const newAppointment = { name, contact, dateTime };
